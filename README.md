...
## 大V投资画像（vip_profiles.json）

在「每日讨论归纳」与「标的提及追踪」之外，把**每天收集的历史发言沉淀为各人的投资画像**（2026-09-24 新增）：LLM 每天把「现有画像 + 当日新增发言」做**重写式修订**，让画像随发言持续演进，替代会过期的 reports 存档（90 天清理）成为长期记忆。

**产出**：`data/vip_profiles.json`（随 workflow 自动提交），每用户一份：
...
```jsonc
{
  ...
  "users": {
    "<user_id>": {
      "name": "紫金陈",
      "profile_max_id": 402640556,     // 画像消费游标（独立于 state 去重游标）
      "last_profile_date": "2026-09-24",
      "profile": {                      // 5 维度锁定 schema，只记稳定特质
        "投资理念": "...", "选股与分析方法": "...", "交易与仓位习惯": "...",
        "关注领域与常谈标的": "...", "风险态度与心理特质": "..."
      },
      "summary": "一句话总评（≤80字）",
      "evolution": ["2026-09-24：初始画像（消费发言 12 条）", "..."]  // 保留最近 60 条
    }
  }
}
```
...
**幂等语义（核心）**
- 待消费发言 = `pending_posts`（上次失败遗留）+ 本轮新增（`profile_max_id` 之后）；**LLM 成功才推进游标并清 pending**，失败/输出走样 → 画像不动、发言转 pending 下轮自动重试，**绝不丢失也不重复消费**；无待消费 → 0 次 LLM 调用（同日重跑自动跳过）。
- 每用户每天最多 1 次调用（复用三级后端链），失败不影响主流程。

**防污染铁律**（迁移自 douban-tracker 画像经验）：画像只记**稳定特质**，严禁行情判断/具体点位/仓位数字/今日操作等时效性内容写入；schema 锁定 5 维度防膨胀；USER_HINTS 黑话词典注入 prompt 帮 LLM 读懂发言（如 谷子地 的 mnp/大波/羊毛），但不照抄进画像。

> 画像为 LLM 依据公开发言的主观归纳，**仅供参考，不构成投资建议**。消费方：cmb-tracker 看板「雪球大V追踪」板块的「大V投资画像」卡片区。
